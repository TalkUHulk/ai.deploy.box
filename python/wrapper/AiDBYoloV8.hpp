...
#ifndef AIDB_AIDBYOLOV8_HPP
#define AIDB_AIDBYOLOV8_HPP
...
public:
    ...
    AiDBYoloV8(const std::string& model, const std::string& backend, const std::string& config_zoo){
    ...
    void forward(py::array_t<uint8_t>& frame_array, int frame_width, int frame_height, py::dict &result) {
        ...
        AIDB::Utility::yolov8_post_process(outputs[0], outputs_shape[0], object, 0.45, 0.35, detect_ins->scale_h());
