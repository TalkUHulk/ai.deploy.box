...
//        ENGINE_ASSERT(nullptr != input)
    ...
    Interpreter *
    Interpreter::createInstance2(const std::string& model, const std::string& lora, const std::string& backend, const std::string& config_zoo) {
        StatusCode status = NO_ERROR;
        aidb_log_init(AIDB_DEBUG, "debug");
        ...
        auto LoRa = node["LoRa"];
        if(!LoRa[lora.c_str()].IsDefined()){
            spdlog::get(AIDB_DEBUG)->error("lora not support! backend:{}, model:{}", backend_lower, model_lower);
            return nullptr;
        }
        model_node["lora"].push_back(LoRa[lora.c_str()]["path"]);
