...
void test_yolov7(AIDB::Interpreter* ins, const cv::Mat& bgr, cv::Mat &result, bool grid=false){
    ...
    *ins >> src_image;
    ...
    if(grid){
        AIDB::Utility::yolov7_post_process(outputs[0], outputs_shape[0], object, 0.45, 0.25, ins->scale_h());
    } else{
        AIDB::Utility::yolov7_post_process(outputs, outputs_shape, object, 0.45, 0.25, ins->scale_h());
    }
...
int main(int argc, char** argv){
    ...
    auto idx = std::string(argv[1]).find("grid");
    bool grid = true;
...
//        writer.open("yolov7.mp4", codec, 25, cv::Size(cap.get(cv::CAP_PROP_FRAME_WIDTH), cap.get(cv::CAP_PROP_FRAME_HEIGHT)));
