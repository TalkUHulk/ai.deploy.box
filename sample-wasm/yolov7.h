...
#ifndef AIDEPLOYBOX_YoloV7_H
#define AIDEPLOYBOX_YoloV7_H

#include "models/YoloV7_tiny.mem.h"
#include "models/YoloV7_tiny.id.h"
#include <iostream>
#include <string>
...
public:
    ...
    int init(){
        _ins = AIDB::Interpreter::createInstance(yolov7_tiny_param_bin, yolov7_tiny_bin, _config);
        return 0;
    }
...
//        _post_process->forward(outputs[0], outputs_shape[0], object, _ins->width(), _ins->height(), _ins->scale_h());
        AIDB::Utility::yolov7_post_process(outputs, outputs_shape, object, 0.45, 0.25, _ins->scale_h());
        return 0;
    }
...
private:
    ...
    std::string _config = "name: \"YoloV7\"\n"
                          "backend: \"wasm\"\n"
                          "num_thread: 2\n"
                          "device: \"CPU\"\n"
                          "PreProcess:\n"
